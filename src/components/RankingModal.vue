...
function handleEdit(id: string) {
  ...
  evaluationStore.fetchEvaluationForEdit(id)
  uiStore.closeModal()
}
...
function handleExport() {
  uiStore.showToast('Funcionalidade de exportação XLS ainda não implementada.', 'info')
}
</script>

<template>
  <!-- ✅ CORREÇÃO: Adicionado 'w-full' para impedir o modal de "espremer" -->
  <BaseModal class="w-full max-w-6xl" v-slot="{ titleId }">
    ...
    <div class="flex flex-wrap items-center justify-between gap-4 border-t border-border-light p-6 bg-secondary">
      ...
      >
        <i class="fa-solid fa-file-excel mr-2"></i> Baixar XLS
      </button>
    </div>
