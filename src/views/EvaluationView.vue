...
// Armazena as respostas (chave = id da pergunta, valor = nota numérica)
...
const maxScore = questions.length * 5 // 4 perguntas * 5 pontos
...
const isFormValid = computed(() => {
  ...
  return form.sectorId && form.responsible && form.date && allAnswered
})

// --- Ciclo de Vida ---
onMounted(async () => {
  // Se houver ID de edição na store, preenche o form
  if (store.editingId && store.currentEvaluation) {
    ...
    if (data.photo_url) {
      imagePreview.value = data.photo_url
    }
    
    // Tenta recuperar as respostas do JSON de observações antigas ou define padrão
    // Nota: Em um sistema ideal, 'answers' seria uma coluna JSONB separada no banco.
    // Aqui assumimos que começa zerado na edição para forçar reavaliação ou precisaria de lógica de parse.
  }
})
...
async function handleSubmit() {
  if (!isFormValid.value) {
    ui.notify('Preencha todos os campos e avalie todos os itens.', 'warning')
    return
  }
  ...
  const payload = {
    ...
    weight: 0, // Implementar lógica de peso se necessário
    observations: formatObservations(), // Junta as respostas com o texto
    image: form.image
  }
...
function formatObservations() {
  // Cria um resumo legível das respostas para salvar no banco
  const details = questions.map(q => {
    const note = answers[q.id] === 5 ? 'OK' : 'Irregular'
    return `${q.text}: ${note}`
  }).join('\n')
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
      <div>
        ...
        <p class="text-sm text-gray-500 dark:text-gray-400 mt-1 ml-1">
          Preencha os dados da inspeção 5S
        </p>
      </div>
              ...
              >
                ...
                <div class="space-y-2 pointer-events-none">
                  ...
                  <p class="text-sm font-medium text-gray-600 dark:text-gray-300">
                    Toque para tirar foto ou enviar
                  </p>
